...
class AccountsSection extends Component {
  ...
  async fetchAccounts() 
  {
    try {
      ...
      var response = await axios.post(`https://ecss-backend-node-backup.azurewebsites.net/accountDetails`, { "purpose": "retrieve"});
  ...
  async fetchAccessRights() 
  {
    try {
      this.setState({ loading: true });
      var response = await axios.post(`https://ecss-backend-node-backup.azurewebsites.net/accessRights`, { "purpose": "retrieve"});
